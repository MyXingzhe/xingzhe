...
uint8_t I2cFunc::I2cReadByteData(uint8_t command)
{
...
int I2cFunc::I2cWriteByteData(uint8_t command, uint8_t value)
{
	union i2c_smbus_data data;
	data.byte = value;
...
int I2cFunc::I2cWriteByte(uint8_t value)
{
	return I2cSmbusAccess(m_fd, I2C_SMBUS_WRITE, value,
	                        I2C_SMBUS_BYTE,NULL);
}

uint8_t I2cFunc::I2cReadBit(uint8_t cmd, uint8_t bits, uint8_t *data)
{
	uint8_t value = I2cReadByteData(cmd);
	*data = value & (1 << bits);
...
int I2cFunc::I2cWriteBit(uint8_t cmd, uint8_t bits, uint8_t data)
{
	uint8_t value = I2cReadByteData(cmd);
 ...
 */
int8_t I2cFunc::I2cReadBits(uint8_t cmd, uint8_t bit_start, uint8_t length, uint8_t *data) 
{
    // 01101001 read byte
 ...
 */
int I2cFunc::I2cWriteBits(uint8_t cmd, uint8_t bit_start, uint8_t length, uint8_t data) 
{
    //      010 value to write
    // 76543210 bit numbers
    //    xxx   args: bitStart=4, length=3
    ...
    return I2cWriteByteData(cmd, value);
