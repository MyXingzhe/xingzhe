...
void Gpio::StartMmap()
{
    ...
    for (i = 0;i < 4;i++)
    {
        ...
        {
            cout << ("gpio_init: gpio mmap failure!\n");
            return;
        }
    }
}
...
int Gpio::SetDir(int connector, int pin, int dir)
{
...
int Gpio::GetDir(int connector, int pin)
{
    unsigned int value;
    int port, dir;
...
int Gpio::SetHigh(int connector, int pin)
{
...
int Gpio::SetLow(int connector, int pin)
{
...
int Gpio::SetIrqMode(int connector, int pin, enum GPIO_IRQ_MODE mode)
{
    int port = (connector == 8)?pin-1:pin+45;
...
bool Gpio::IsHigh(int connector, int pin)
{
    int port = (connector == 8)?pin-1:pin+45;
    StartMmap();
